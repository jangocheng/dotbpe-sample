    ...
    public class MathService : ServiceActor
    {
        ...
        public Task<AmpMessage> AddAsync(AmpMessage reqMsg){
            ...
            if(req !=null){
                ...
                Logger.LogDebug("{0}${1}${2}  ,code=0  ,req={3}  ,res={4}",reqMsg.ServiceId,reqMsg.MessageId,reqMsg.Sequence,req.ToString(),res.ToString());
            }
            else{
                rsp.Code = ErrorCodes.CODE_INTERNAL_ERROR;
                Logger.LogError("{0}${1}${2}  ,code={3}  ,req={4}  ,res=null",reqMsg.ServiceId,reqMsg.MessageId,reqMsg.Sequence,rsp.Code,req.ToString());
            }
